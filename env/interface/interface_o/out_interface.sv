...
else $error("INTERFATA_IESIRE: a picat asertia asertia_stable_data");
...
property valid_fall;
	...
	$fell (valid_o) |-> $past(valid_o && ready_i); //se poate simplifica, se considera valid_o ca fiind deja activ
endproperty

asertia_valid_fall: assert property (valid_fall)
else $error("INTERFATA_IESIRE: a picat asertia valid_fall");
...
else $error("INTERFATA_IESIRE: a picat asertia correct_data");
